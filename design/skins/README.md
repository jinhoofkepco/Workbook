...
현재 앱은 문제집 선택 화면의 책 표지/책등, 페이지 프레임, 제목 프레임, 선반 밴드, 소단원 행, 문제 화면 헤더/이전/다음/힌트/제출 버튼, 마스터 `마` 버튼 프레임에 스킨을 적용합니다. 학생용 하단 진도/문제 메뉴는 제거되었으므로 하단 메뉴판은 필수 에셋이 아닙니다.

프롬프트를 새로 만들 때는 `paper_album_v1/prompts.md`의 좌표 기준을 먼저 유지하는 것을 권장합니다. 특히 상단 0~160px, 하단 2380~2560px, 책 표지 그리드 중앙 영역, 문제 화면 도구 레이어 영역을 명확히 비워 두면 생성 이미지가 앱 UI와 덜 충돌합니다. 버튼 프레임 PNG 안에는 글자/숫자/버튼 문구를 넣지 말고, 앱이 텍스트를 얹을 수 있게 중앙을 비워 둡니다.
